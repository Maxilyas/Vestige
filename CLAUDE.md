...
- ⬜ **Phase 9 — Refonte salles compactes (mini-Metroidvania)** — Pivot architectural : salles **960×540 fixes** (= canvas), caméra figée (pas de scroll), 12-18 salles par étage au lieu de 5-7, refonte from scratch des pools de salles biome par biome. Densité forcée par contrainte spatiale + densification visuelle (layers parallaxe ×2, lighting dynamique, props et micro-animations partout). Phase de transition : tout le combat / loot / Vestiges / Fondeur reste intact, seul le pipeline spatial + rendu est refondu. Sous-phases : 9.1 fondations (grille étendue 6×5, cible 12-18 salles ✅), 9.2 salle test ultra-dense (Ruines), 9.3+ migration biome par biome (Ruines → Halls → Cristaux → Voile → Cœur).
...
- **Dernière étape franchie** : Phase 9.1 — Fondations mini-Metroidvania. Grille spanning tree étendue 5×5 → 6×5, probas branches/extensions/boucles augmentées (0.55→0.85, 0.55→0.65, 0.10→0.18), post-pass de remplissage qui force des branches additionnelles si le graphe est sous 12 nœuds. Cible : 12-18 salles par étage. Étages 1-4 (migrés) passent en grille 6×5. Non-destructif : aucune salle existante touchée, MapScene auto-cadre sa grille donc rien à adapter. Phase 9 sera complétée par 9.2 (caméra figée + dims 960×540 + 1 salle test ultra-dense) puis migration biome par biome.
- **À tester en navigateur** : étages 1-4 → vérifier que les runs A→BOSS comportent 12-18 salles (au lieu de 5-9), avec densité de branches et boucles élevée. Carte M affiche correctement la nouvelle taille de grille. Étages 5-10 toujours sur ancien système (graphe 5-noeuds, sera migré dans 9.3+).
- **Prochain chantier** : Phase 9.2 — caméra figée + dims fixes 960×540 (via flag sur topographie) + 1ère salle test Ruines ultra-dense (7+ layers parallaxe, lighting dynamique, props denses) pour valider le feel densité avant migration biome.
