...
| **5b.1** | ✅ | **Vestiges — fondations.** Nouvelle catégorie de loot : Vestige (drop boss exclusif). 3 slots dédiés `{geste, maitrise1, maitrise2}` séparés de tête/corps/accessoire. 10 Vestiges éditorialisés (5 Maîtrises passives + 4 Gestes actifs + 1 Artefact). Drop signature au boss, no-redrop si déjà possédé. Layout 2 colonnes dans `InventaireScene` (Équipement \| Vestiges). Panneau détail refondu en layout horizontal responsive. Capacités pas encore câblées (touche V + flags passifs viennent en 5b.2). |
| **5b.2** | ⬜ | **Vestiges — capacités.** Câbler touche V (`InputSystem` → intention `geste`). Système `GesteSystem` modulaire. 4 Gestes : onde du Glas (AOE), filet de Cendre (tir rectiligne), œil-témoin du Sans-Fin (tir homing), sève d'Hydre (dash invulnérable). 4 Maîtrises actives : double-saut, slow-mo parry, renaissance (auto-revive 1×), révélation totale T3. Cooldown visuel sur slot Geste. |
...
- **Dernière étape franchie** : **Phase 5b.1 — Vestiges, fondations**. Nouvelle catégorie de loot : **Vestige** (drop boss exclusif, jamais en coffre / Marchand). Nouveau fichier `data/vestiges.js` avec 10 entrées (5 Maîtrises passives + 4 Gestes actifs + 1 Artefact étage 10). Trois sous-types : `maitrise` (passif), `geste` (actif touche V), `trophee` (Artefact). Famille `vestige` (cramoisi) + `artefact` (doré) ajoutées aux `COULEURS_FAMILLE`. Résolveur universel `getItemOuVestige(id)` qui interroge ITEMS puis VESTIGES. `InventaireSystem` étendu : 3 slots `vestiges_equipes` = `{geste, maitrise1, maitrise2}`, helpers `equiperVestigeDepuisInventaire` (auto-route par sousType), `desequiperVestige`, `possedeVestige`, `aVestigeFlag(flag)`. `LootSystem.calculerStats` accepte un 3e param `vestiges` et additionne leurs effets. `GameScene._dropBossTier3` → `_dropBossVestige` : lookup `vestigePourEtage(etage)`, cube cramoisi + étincelles dorées, no-redrop si déjà possédé. HUD `UIScene` : 3 slots Vestige sous l'équipement (label cramoisi, labels courts GESTE/MAÎT. I/MAÎT. II). `InventaireScene` complètement réorganisée en layout 2 colonnes (Équipement gauche / Vestiges droite) + bande ressources remontée en haut + grille décalée à y=270. `PanneauDetail` refondu en layout horizontal responsive : emblème + nom alignés à l'horizontale, description courte centrée, `yEff` dynamique calculé selon la hauteur réelle de la description → place pour 5-6 effets confortablement. Emblèmes `vestige` (sceau cramoisi gravé + point doré) et `artefact` (couronne dorée 3 pointes) ajoutés à `EmblemeFamille`. **Capacités (touche V + flags passifs) pas encore câblées — c'est 5b.2.**

- **Précédentes étapes** : **Phase 5a** Sceaux d'étage (10 emblèmes HUD persistés en localStorage, anim pop + halo doré au boss-kill). **Phase 4** Étages déterministes (`data/etages.js` + `CarteMemoire.js`). **Phase 3g** Rareté & polish (probas par étage + auras + drops boostés). **Phases 3d + 3e + 3f** Bestiaires Cristaux / Voile / Reflux (18 archétypes + `PerceptionSystem` + `CloneIllusionSystem` + tile fissure). **Phase 3c** Halls Cendrés (10 ennemis, `EnvironmentMutators` gele/mur_feu, Projectile.effetImpact). **Phase 3b** Ruines basses (6 archétypes + SpawnerSystem + PerceptionCloud + tile glissante). **Phase 3a** Fondations bestiaire (split data + registry comportements/visuels + RaritySystem standalone).
...
- **Prochain chantier** : **Phase 5b.2 — Vestiges, capacités**. Brancher la touche V (`InputSystem` → intention `geste`) et créer `systems/GesteSystem.js` (registry modulaire des 4 Gestes : onde du Glas / filet de Cendre / œil-témoin du Sans-Fin / sève d'Hydre). Implémenter les 4 flags Maîtrise dans GameScene : `doubleSaut` (gestion `jumpsRestants` + reset au touche-sol), `slowMoParry` (hook au parry réussi → `physics.world.timeScale = 0.35` 1.5s), `renaissance` (hook avant `prendreDegats` → si Résonance→0 et flag actif et pas encore utilisé ce run → reset à 1 + invu 2s + marquer utilisé), `revelationTotale` (hook IdentificationSystem). Cooldown visuel sur slot Geste du HUD. Puis **5c** = écran de victoire au 10e sceau + drop spécial de l'Artefact (déjà côté data, juste à brancher).
