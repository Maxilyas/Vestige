...
- **Dernière étape franchie** : Phase 8.3 — Refonte architecturale + mur_secret + gouffres mortels. Les 25 salles Halls ont été REFAITES from scratch avec une vraie architecture : `mur(x, yHaut, yBas)` pour les murs verticaux, `murLateralGauche/Droit` pour fermer les bords selon ports actifs, `plafondCathedrale` (auto-tag 'structurel') pour voûtes ornementales non-walkable. Brasiers maintenant placés sur des estrades/foyers/niches (visuellement justifiés), plus sur sol plat. Nouveau obstacle `mur_secret` : visuellement IDENTIQUE à une plateforme du biome (ré-utilise `peindreOrnementPlateforme`), 4 hits pour casser, fissures fines apparaissent seulement après 2 hits — pure découverte Metroidvania. Nouveau flag salle `gouffreMort: true` (cf. `halls_cascade_pierres`, `halls_pont_braise`) + helper `_declencherMortGouffre` dans GameScene : sol non-continu = chute mortelle. Bugs fixés : brasier sans overlap dégâts, mur_explosif sans collider/sans filtre attaque (lignes 510/1081 GameScene). Validateur 47/47 OK (ruines_grimpeur attendu KO).
- **À tester en navigateur** : étages 2 + 3 + 4 (runs A→BOSS multi-seeds). Vérifier que les brasiers font dégâts en phase feu ; que les murs explosifs bloquent + dégâts à la rupture ; que les murs secrets sont indiscernables (taper aléatoirement les murs des Halls = découverte coffres) ; que la chute dans gouffre = mort instant (cf. `halls_pont_braise`, `halls_cascade_pierres`). Étages 5-10 toujours sur ANCIEN système topographies.
- **Prochain chantier** : (a) migrer étages 5-6 (Cristaux Glacés — biome cristaux mnésiques, mécanique à définir) ; (b) sanctuaires boss étages 6/7/8/10 ; (c) Phase 6.x polish équilibrage.
