...
| **3d-3f** | ⬜ | **Bestiaires Cristaux / Voile / Reflux.** 6 archétypes innovants par biome (18 au total). PerceptionSystem complet (vision blur / contrôles inversés / gravity flip) à partir de 3d. |
...
- **Dernière étape franchie** : **Phase 3c — Bestiaire Halls Cendrés**. 10 ennemis biome 3-4 (4 basics + 6 innovants : Chandelier Vivant, Brûleur Lent, Cendre-Tisseuse, Ardent Miroir, Soupir Glacial, Tisseur d'Embrasement). `EnvironmentMutators` étendu (`gele` variante glace + `mur_feu` DPS gate -4/500ms). Projectile supporte `effetImpact(scene, player)` pour effets additionnels (web-spinner immobilise via flag `_immobiliseJusqu`). GameScene : check immobilise dans mouvement + slippery (decay 0.98 + sluggish accel 92/8 blend) via flag `_tileEffectGlissant`. Bug fix : mur de feu ancré sur `player.y` (pas `enemy.y`) car Tisseur peut être sur plateforme différente.

- **Précédente étape** : **Phase 3b — Bestiaire Ruines basses + 3 mini-systèmes transversaux**. 10 ennemis biome 1-2 (4 basics + 6 innovants). Nouveaux systèmes : `SpawnerSystem`, `EnvironmentMutators` (tile glissante), `PerceptionCloud`. Refactor `EnemyVisuel.js` en registry. Event `enemy:spawn` relayé via Enemy.js. Pool biome pondéré 50/50.

- **Avant-précédente** : **Phase 3a — Fondations bestiaire**. Split `data/enemies.js` en `data/enemies/{ruines,halls,cristaux,voile,reflux,index}.js`. Registry pour `EnemyComportements/` avec `_registry.js` séparé (résout circular imports). `RaritySystem.js` standalone (4 tiers, probas désactivées commun=100% jusqu'à Phase 3g).
...
- **Prochain chantier** : **Phase 3d — Bestiaire Cristaux Glacés**. 6 archétypes innovants étages 5-6 : Cristal-Prisme (vision-distorter, brouille la vue dans rayon), Givre-Tisseur (floor-mod, gèle tile sous joueur), Éclat-Multiplicateur (mirror-clone, spawne clones illusoires à chaque hit), Reflet-Double (mirror-being, mimétise dernier coup joueur), Anneau de Glace (orbital, éclats orbitant), Polariseur (control-inverter, inverse contrôles 1s). **Première implémentation `PerceptionSystem` complet** : vision blur (Phaser PostFX), contrôles inversés (wrapper InputSystem), clones illusoires (`CloneIllusionSystem`), projectiles orbitaux. Splitter pattern réutilisé (Idole Fissurée déjà basic → upgrade).
