...
- **Dernière étape franchie** : Phase 9.2 — Plomberie salle compacte + 1ère salle test. Flag `dimsCanvas: true` sur la topographie force `WorldGen.genererSalle` à utiliser dims 960×540 (= canvas) au lieu des dims XL legacy. `GameScene` branche un mode caméra figée : `cam.stopFollow()` + `setScroll(0, 0)` + zoom-out désactivé (rien à voir hors champ). Mode mixte transitoire : les salles legacy (47 existantes) gardent leur scroll/follow, seules les NOUVELLES salles flaggées passent en mode compact. Salle test créée : `ruines_atrium_effondre.js` — 5 niveaux verticaux empilés dans 540 px (sol → palier 430 → palier 360 → passerelle centrale 290 → palier sommet 220 → corniches 150), voûte cathédrale décorative, coffre forcé sur la passerelle haute, `unique: true` + ajout au pool Ruines.
- **À tester en navigateur** : étage 1 ou 2 → rerolls multiples (touche K + retour Cité + vortex) pour voir tomber sur l'atrium. Vérifier : (a) la salle remplit exactement l'écran sans scroll caméra, (b) le joueur peut zigzag sur les 5 niveaux verticaux pour atteindre la passerelle haute + coffre, (c) la transition vers/depuis cette salle compacte se fait proprement, (d) feel exploration mini-Metroidvania (12-18 salles/étage) globalement satisfaisant.
- **Prochain chantier** : Phase 9.3 — migration Ruines basses : refonte from scratch d'un pool de ~20-25 salles compactes 960×540 (remplace l'ancien pool). Si feel validé, on enchaîne sur Halls Cendrés. Densification visuelle (lighting + layers parallaxe ×2) à intégrer au pipeline biome.
