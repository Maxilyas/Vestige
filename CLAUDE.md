...
- **Dernière étape franchie** : **Phases 3d + 3e + 3f — Bestiaires Cristaux / Voile / Reflux**. 18 archétypes innovants ajoutés (6 par biome). 2 nouveaux systèmes : `PerceptionSystem` (flags vision flou / contrôle inversé / gravité inversée / parry lock / vulnérabilité, overlay sombre full-screen pour vision flou), `CloneIllusionSystem` (clones visuels via réutilisation creerVisuelEnnemi). `EnvironmentMutators.ajouterTileFissure` (tile qui s'élargit puis explose AOE -8). GameScene patches : wrapper resonance pour vulnérabilité ×1.5, swap inputs gauche/droite, body.gravity.y = -worldGravity (net=0 flottement), parryImmune dans contactEnnemi, lastAttaqueAt pour Reflet-Double. Bug fixes : Cristal-Prisme overlay simplifié (toggle setVisible), gravité inversée moins extrême (flottement au lieu d'expulsion).

- **Précédente étape** : **Phase 3c — Bestiaire Halls Cendrés**. 10 ennemis biome 3-4 (4 basics + 6 innovants). `EnvironmentMutators` étendu (`gele` + `mur_feu`). Projectile supporte `effetImpact`. GameScene : check immobilise + slippery.

- **Avant-précédentes** : **Phase 3b** Ruines basses (6 archétypes + SpawnerSystem + PerceptionCloud + tile glissante). **Phase 3a** Fondations bestiaire (split data + registry comportements/visuels + RaritySystem standalone).
...
- **Prochain chantier** : **Phase 3g — Rareté & polish**. Activer les probas Commun/Élite/Rare/Légendaire dans `RaritySystem` (actuellement commun=100%). FX auras visuelles par tier (halo pulsant Élite, aura argentée Rare, halo cramoisi massif + screen-shake Légendaire). Tuning fréquences par biome. Hook signature-drop pour Légendaires (drop unique par enemy.id, modulaire pour future refonte recettes).
