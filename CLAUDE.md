...
- **Dernière étape franchie** : Phase 9.3c — Pool compact Ruines COMPLET (+15 salles). Couvre toutes les configs spanning tree : 1 carrefour NSEO (`ruines_carrefour_compact`, fallback Ruines compact), 2 NS verticaux (`puits_compact` zigzag fluide, `cheminee_compact` voie centrale ressorts + sols effrités), 4 coins (NE tour de garde / NO belvédère / SE descente / SO caveau avec éboulis), 4 T-shapes (NEO forum 3 voies / SEO embranchement / NSO passage triple ouest / NSE est), 4 impasses (O sanctuaire scellé / E atelier scellé / N corniche oubliée / S caveau profond). Total = 16 salles compactes Ruines (atrium + 4 OE + 11 nouvelles + carrefour fallback). Flag `MODE_COMPACT_ONLY` limité au biome `ruines_basses` → étages 1-2 100% compact, étages 3-4 Halls XL inchangés. `FALLBACK_PAR_BIOME['ruines_basses']` pointe désormais sur `ruines_carrefour_compact`. Validateur : 16/16 OK sur le pool compact Ruines (seul `ruines_grimpeur` legacy reste KO comme attendu).
- **À tester en navigateur** : étage 1 ou 2 → tu devrais voir UNIQUEMENT des salles compactes 960×540 (caméra figée, salle = écran, lecture immédiate). Le spanning tree génère 12-18 salles avec 5-8 configs différentes (OE/NS/coins/T/impasses). Vérifier feel exploration mini-Metroidvania + chaque type de salle (verticalité puits, virages coins, arènes combat, impasses coffre). Étage 3-4 (Halls) toujours en XL legacy (migration 9.4+).
- **Prochain chantier** : Phase 9.3d (densification visuelle Ruines : lighting + parallaxe ×2) OU Phase 9.4 (migration Halls Cendrés en 960×540) selon priorité user. Refonte arena boss + arene_ouverte entrée Ruines aussi à prévoir pour étage 100% compact (cinématiquement les transitions A et BOSS restent XL à l'étage 1-2).
