...
- **⚠️ Review à faire** : la Phase 6 (livraison initiale + retrait du legacy) **n'a pas été testée bout-en-bout en navigateur**. Le code compile / les imports sont cohérents / les chemins critiques ont été tracés mentalement, mais une session de jeu réelle reste à faire. Points sensibles à valider en priorité : (a) un nouveau run depuis MenuScene ne crashe pas, (b) les coffres droppent bien des instances avec losange + score chiffré, (c) la barre Garde apparaît dès qu'un item à gardeMax > 0 est équipé et absorbe les dégâts avant la Résonance, (d) les sorts 1/2/3 se déclenchent quand un item équipé a un sortId révélé, (e) Fondeur → 3 onglets fonctionnels, le résultat de combinaison/reroll s'ajoute bien à l'inventaire, (f) Identifieur révèle exotique/sort/signature et le PanneauDetail se rafraîchit après, (g) Marchand : achat retire l'instance de la vitrine + ajoute à l'inventaire, fragmentation rend les bons fragments, (h) AuraItem apparaît visuellement au-dessus de score 70 équipé.

- **Prochain chantier** : **Phase 6 polish / tests live + corrections**. Reste à faire après la review : système d'Échos (re-respawn salles nettoyées en Élite + drop bonus) non implémenté, repoussé pour ne pas changer le mini-jeu sous le coude.
