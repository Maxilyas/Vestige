...
- ✅ Refonte boss biomes 3-4 (Cristaux/Voile, side-scroll) — `systems/BossCristauxVoile.js` (+ helpers partagés `BossSideScrollHelpers.js`) : é5 Chœur Pétrifié (séquence + verglas), é6 Jumeaux Résonants (deux corps liés + fusion), é7 Tyran-Miroir (miroir + inversions de gravité), é8 **Souverain du Voile = CLIMAX** (danmaku + gravité + échos + parade + arène qui rétrécit). Arènes 960×540 caméra figée. **→ les 10 boss sont désormais des encounters uniques.**
...
- **10 boss = 10 encounters BESPOKE** (verbes uniques, 3 phases + secret phase chacun) : é1-8 side-scroll (`BossRuinesHalls.js` + `BossCristauxVoile.js`, helpers `BossSideScrollHelpers.js`), é9-10 top-down (`BossCoeurReflux.js`). Plus AUCUN pattern générique colosse/tisseur/hydre utilisé (legacy, conservés mais débranchés). Gating porte E salle BOSS. Drop : instance score 80+ garantie + 10-25 Sel + 3 Fragments. Boss étage 10 → cinématique fusion → FinScene.
...
- **LES 10 BOSS SONT REFONDUS** (chacun unique, 3 phases + secret phase). Side-scroll é1-8 dans `BossRuinesHalls.js` (é1-4) + `BossCristauxVoile.js` (é5-8), helpers partagés `BossSideScrollHelpers.js` (gate de vuln + secret, zones, ondes, gravats) ; event `joueur:attaque` (GameScene) pour les destructibles ; arènes é5-8 en **960×540 = caméra figée**. Top-down é9-10 dans `BossCoeurReflux.js`.
  - **é1 Cariatide** (DÉTRUIRE piliers→voûte) · **é2 Colosse de Sel** (GRIMPER corniches→nœuds) · **é3 Porteur de Lanternes** (ÉCLAIRER vasques dans le noir) · **é4 Effigie Ardente** (KITER vers l'eau).
  - **é5 Chœur Pétrifié** (SÉQUENCE statues qui chantent + sol verglacé ; secret = faux signaux) · **é6 Jumeaux Résonants** (DEUX CORPS liés, équilibrer les PV ; secret = fusion) · **é7 Tyran-Miroir** (MIROIR de tes mouvements → le piéger dans les lasers + **inversions de gravité** via pendule Voile ; secret = symétrie centrale) · **é8 Souverain du Voile** = **CLIMAX** (danmaku + gravité + **échos de toi** + Orbe à parer + arène qui rétrécit ; secret « Le Reflux te réclame » = micro-fenêtres post-flip).
  - **é9 Doyen « Procès »** + **é10 Cœur « Polymorphe »** (top-down) → FinScene.
  - Tout validé navigateur (mécaniques, secret phases, mort→boss:dead, régressions OK, 0 erreur). NB test : la boucle Phaser se met en pause hors-focus → tester les mécaniques loop-dépendantes via `b.update()` synchrone.
- **Reste** : feel-test/équilibrage des 10 boss (hp, durées de fenêtres, densité danmaku é8) ; gros polish + densité/difficulté des SALLES ; salles catalogue Cœur §6 ; audio Tone.js. Dédup possible : `BossRuinesHalls.js` pourrait importer `BossSideScrollHelpers.js` (copies locales conservées, sûres).
