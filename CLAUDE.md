...
- **Dernière étape franchie** : **Phase 5c.1 — MenuScene de démarrage**. Nouvelle scène `MenuScene` lancée au boot (avant GameScene) via `main.js` réordonné. Ambiance "ruines au crépuscule" : `poserCiel('normal')` + `poserSilhouettesLointaines` (rng fixe pour composition stable au reload), bande de sol sombre, 4 nuages de brume bleutée animés en boucle horizontale, 3 lanternes Présent éteintes (`peindreLanterne`) avec reflet doré scintillant additif (Graphics positionné, fillCircle(0,0,r), tween scale autour du centre), particules de cendre dorées ascensionnelles via `time.addEvent`, vignette aux bords. Titre `VESTIGE` Georgia 56px doré bold avec `setShadow(0,0,'#ffd070',12,true,true)` + halo additif radial pulsant (alpha yoyo 3s). Sous-titre italique poétique (`La mémoire marche entre deux mondes`, variante post-fin `Le souvenir reformé t'attend encore`). Étoile dorée 4-branches additive à droite du titre si `localStorage.vestige_fin_atteinte_v1` (Graphics positionné, dessin à (0,0), tween scale yoyo). 3 boutons painterly empilés verticalement : `Nouvelle partie` (full opacity, hover bleu-violet + texte plus clair), `Continuer` (grisé option C — fond alpha 0.5, bord 0x4a4258, hint `(bientôt)` à droite — pas interactive), `Quitter` (full opacity → fade noir + écran d'adieu). Crédit `— prototype — VESTIGE —` en bas. ENTRÉE/ESPACE = raccourci Nouvelle partie. **"Nouvelle partie"** itère `registry.getAll()` et remove chaque clé pour repartir d'une seed neuve (sinon état du run précédent restait actif après FinScene → Quitter → Menu). Markers localStorage : `vestige_run_actif_v1` posé dans `GameScene.create()` à la première seed générée, cleared dans `FinScene.create()`. `FinScene._quitter()` route maintenant vers `MenuScene` au lieu d'un écran d'adieu local. `main.js` expose `window.game = new Phaser.Game(...)`.
...
- **Prochain chantier** : **Phase 5c.3 — Polish cooldown Geste HUD**. Quand un Vestige Geste est équipé (slot Geste rempli), afficher le cooldown sur le HUD : overlay sombre tournant (sweep horloge) sur l'emblème du Geste pendant la période de cooldown + label numérique du temps restant (centième de seconde). `GesteSystem` enregistre déjà `scene._gesteCdJusqu` (à vérifier) — UIScene consomme via tick. Polish purement visuel, pas de changement de gameplay.
