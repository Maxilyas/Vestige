...
- **Dernière étape franchie** : Phase 9.3b — Pool OE compact Ruines (+4 salles). Chaque salle = un *geste* clair inspiré d'un classique du genre : `ruines_couloir_brise` (combat propre style Hollow Knight, 4 paliers en quinconce), `ruines_escaliers_effrites` (timing sols qui s'effondrent style Celeste, coffre sommet récompense montée), `ruines_arene_pieux` (combat sur sol piégé style Castlevania, 4 pieux sol + refuge central élevé + coffre haut), `ruines_arene_ressorts` (mobilité aérienne dynamique, pyramide 4 niveaux + 2 ressorts pour raccourcir montée). Toutes 5 salles compactes Ruines (atrium + 4 nouvelles) passent le validateur. Avec l'atrium, pool OE Ruines compact = 5 salles → sur étage 1 (12-18 salles main), 3-5 nouvelles salles compactes se mélangeront aux salles XL legacy. Mixité gérée par la plomberie 9.2.
- **À tester en navigateur** : étage 1 ou 2 → vérifier que les 5 nouvelles salles compactes (atrium + 4 nouvelles) apparaissent dans le run, que chacune a un *feel* distinct (combat propre / timing fragile / sol piégé / mobilité aérienne / verticalité), et que la transition entre salles compactes et salles XL legacy se fait proprement (fade noir). **Mode debug temporaire** : flag `MODE_COMPACT_ONLY = true` dans `src/data/salles/_index.js` force le tirage spanning tree à ne sélectionner que les salles compactes (les configs non couvertes — NS/T/NSEO — se rabattent sur le fallback carrefour XL). À désactiver une fois Phase 9.3c complète. Étages 5-10 non affectés (pins éditoriaux conservés).
- **Prochain chantier** : Phase 9.3c — refonte des configurations restantes (NS verticaux, coins, T, impasses) en 960×540 compact. ~10-12 salles supplémentaires pour couvrir toutes les configs de portes du spanning tree Ruines. Puis Phase 9.3d (densification visuelle) avant migration Halls/Cristaux/Voile/Cœur.
